...
import postPrepaymentReq from "../../data/salaryTable/postPrepayment.ts";
import putPrepaymentReq from "../../data/salaryTable/putPrepayment.ts";
import { useSelector } from 'react-redux';
import { selectAccessToken } from "../../store/thisUserReducer";

const EditPrepayment = ({ user, onClose }) => {
    ...
    const [numberValue, setNumber] = useState(user.prepayment.prepayment);
    ;
    ...
    const addPrepayment = async () => {
        const UserData = await postPrepaymentReq({
    ...
    const updatePrepayment = async () => {
        const UserData = await putPrepaymentReq({
            value: Number(numberValue),
            date: date
        },user.prepayment._id, token);
        console.log(UserData);
        if (!UserData.message) {
            onClose();
        } else {
